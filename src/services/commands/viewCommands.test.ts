...
vi.mock("@/services/terminal/toggleTerminalFocus", () => ({ toggleTerminalFocus }));
...
describe("registerViewCommands — full command set", () => {
  ...
  it("registers all 34 view/lint commands", () => {
    ...
    expect(ids).toContain("view.toggleTerminalFocus");
...
describe("view command behavior", () => {
  ...
  it("toggleTerminalFocus delegates to the toggleTerminalFocus service", async () => {
    await executeCommand("view.toggleTerminalFocus");
    expect(toggleTerminalFocus).toHaveBeenCalledTimes(1);
  });
